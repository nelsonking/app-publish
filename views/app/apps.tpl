							...
							<table>
								...
								<tr>
									<td>
										...
										<div class="text drag-state"><span id="_drop1">拖拽到这里上传</span><span id="_drop2">快松手</span>
										</div>
									</td>
								</tr>
								</tbody>
				...
				{{range $index,$apps := .listApps}}
					<div class="col-xs-4 col-sm-4 col-md-4 app-animator ng-scope">
						<div class="card app card-ios">
							{{if eq $apps.Type 1}}
								<i class="type-icon icon-android"></i><div class="type-mark"></div>
							{{else}}
								<i class="type-icon icon-apple"></i><div class="type-mark"></div>
							{{end}}
							...
							<br><p class="appname"><i class="icon-owner"></i><span class="ng-binding"> {{$apps.Name}} </span></p>
							<table>
								<tbody>
									{{ if eq $apps.Type 1}}
										<tr><td class="ng-binding">应用平台：</td><td><span class="ng-binding"> 安卓 </span></td></tr>
									{{else}}
										<tr><td class="ng-binding">应用平台：</td><td><span class="ng-binding"> 苹果 </span></td></tr>
									{{end}}
									<tr><td class="ng-binding">应用标识：</td><td><span class="ng-binding">{{$apps.BundleId}}</span></td></tr>
									<tr><td class="ng-binding">最新版本：</td>
										<td><span>{{$apps.VersionCode}}</span> <span class="ng-binding">{{$apps.BundleVersion}}</span></td>
									</tr>
									<tr><td class="ng-binding">应用大小：</td><td><span class="ng-binding">{{$apps.Size | FormatBytes}}</span></td></tr>
									<tr><td class="ng-binding">上传时间：</td><td><span class="ng-binding">{{FormatTimeStamp $apps.CreatedAt "m-d H:i"}}</span></td></tr>
								</tbody>
							</table>

							<div class="action">
								<a class="ng-binding" href="/apps"><i class="icon-pen"></i> 管理</a>
								<a href="/app/{{$apps.Id}}" target="_blank" class="ng-binding"><i class="icon-eye"></i> 预览</a>
